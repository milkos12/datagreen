...
const { Classification, Company, User } = require('../models');
...
router.get('/', asyncHandler(async (req, res) => {
    const allClassifications = await Classification.findAll({
        include: [
            {
                model: Company,
                as: 'company',
...
router.get('/:id',
    validate([
        param('id').isUUID().withMessage('Classification ID is invalid')
    ]),
...
router.put('/:id',
    ...
    asyncHandler(async (req, res) => {
        ...
        if (created_by) {
            ...
            classification.created_by = created_by;
        }

        // Actualizar otros campos
        classification.name = name !== undefined ? name : classification.name;
...
router.delete('/:id',
    ...
    asyncHandler(async (req, res) => {
        ...
        res.json({ message: 'Classification successfully deleted' });
    })
);
